...
// TODO: Write without using FParsec.
...
let nodesFrom (source: string) =
    ...
    let readTalk lineContent =
        ...
        and collectSpeech currentRev retRev fullContent =
            ...
            | '}' :: rest -> collect [] ((currentRev |> List.rev |> String.ofList |> TalkToken.Speech) :: retRev) rest
        ...
        let rec loop content =
            ...
            if segments'.Length > 1 then
                ...
                let headSegment, intermediateSegments, lastSegment =
                    segments'.Head, segments'.Tail |> Seq.take (Seq.length segments' - 2), segments'.Tail |> Seq.last
